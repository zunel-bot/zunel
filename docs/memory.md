...
- a local git-backed history under `memory/.git/` to track changes to those durable files
...
jq -r 'select(.content | ascii_downcase | contains("keyword")) | .content' \
  ~/.zunel/workspace/memory/history.jsonl
```
